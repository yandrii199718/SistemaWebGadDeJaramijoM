...
$html = '<div></div><table cellpadding="4" style="border: 1px solid '.$colorE.'; font-size: 12px;">
		<tr>
			...
			<th width="7%" style="border: 1px solid '.$colorE.';">Sbn</th>
			<th width="15%" style="border: 1px solid '.$colorE.';">Nombre equipo</th>
      <th width="8%" style="border: 1px solid '.$colorE.';">Modelo</th>
      <th width="14%" style="border: 1px solid '.$colorE.';">Manual</th>
			<th width="18%" style="border: 1px solid '.$colorE.';">Observación</th>
      <th width="10%" style="border: 1px solid '.$colorE.';">marca</th>
      <th width="10%" style="border: 1px solid '.$colorE.';">area</th>
			<th width="10%" style="border: 1px solid '.$colorE.';">sede</th>
		</tr>';



//echo '<pre>'; var_dump($usuarios); exit;
foreach($equipos as $key => $dato){

  $codigo = $dato["codigo"];
  $sbn = $dato["sbn"];
  $nombreEquipo = $dato['nombre_equipo'];
  $modelo = $dato['modelo'];
  $manual = $dato['manual'];
  $observacion = $dato['observacion'];
  $nombreMarca = $dato['nombre_marca'];
  $nombreArea = $dato['nombre_area'];
  $nombreSede = $dato['nombre_sede'];
  ...
  $html .= '<tr style="font-size: 12px; color: #2E2E2E;">
    ...
    <td style="border: 1px solid '.$colorE.';">'.$sbn.'</td>
    <td style="border: 1px solid '.$colorE.';">'.$nombreEquipo.'</td>
    <td style="border: 1px solid '.$colorE.';">'.$modelo.'</td>
    <td style="border: 1px solid '.$colorE.';">'.$manual.'</td>
    <td style="border: 1px solid '.$colorE.';">'.$observacion.'</td>
    <td style="border: 1px solid '.$colorE.';">'.$nombreMarca.'</td>
    <td style="border: 1px solid '.$colorE.';">'.$nombreArea.'</td>
    <td style="border: 1px solid '.$colorE.';">'.$nombreSede.'</td>
  </tr>';
