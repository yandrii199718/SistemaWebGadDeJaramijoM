...
$html = '<div></div><table cellpadding="4" style="border: 1px solid '.$colorE.'; font-size: 12px;">
		<tr>
			...
			<th width="8%" style="border: 1px solid '.$colorE.';">Codigo</th>
			<th width="12%" style="border: 1px solid '.$colorE.';">Nombre equipo</th>
			<th width="15%" style="border: 1px solid '.$colorE.';">nombre usuario</th>
			<th width="20%" style="border: 1px solid '.$colorE.';">Actividad</th>
      <th width="8%" style="border: 1px solid '.$colorE.';">Fecha operacion</th>
			<th width="10%" style="border: 1px solid '.$colorE.';">Fecha mantenimiento</th>
			<th width="10%" style="border: 1px solid '.$colorE.';">T. Mantenimiento</th>
			<th width="12%" style="border: 1px solid '.$colorE.';">Estado</th>
		</tr>';



//echo '<pre>'; var_dump($usuarios); exit;
foreach($mantenimientos as $key => $dato){

  $codigo = $dato["codigo"];
  $equipo = $dato["nombre_equipo"];
  $nombres = $dato['nombres'];
  $actividad = $dato['actividad'];
  $fechaOperacion = $dato['fecha_operacion'];
  $fechaMantenimiento = $dato['fecha_mantenimiento'];
  $tipoMantenimiento = $dato['tipo_mantenimiento'];

  $estado = $dato['estado_alerta'];
  ...
  $html .= '<tr style="font-size: 12px; color: #2E2E2E;">
    ...
    <td style="border: 1px solid '.$colorE.';">'.$equipo.'</td>
    <td style="border: 1px solid '.$colorE.';">'.$nombres.'</td>
    <td style="border: 1px solid '.$colorE.';">'.$actividad.'</td>
    <td style="border: 1px solid '.$colorE.';">'.$fechaOperacion.'</td>
    <td style="border: 1px solid '.$colorE.';">'.$fechaMantenimiento.'</td>
    <td style="border: 1px solid '.$colorE.';">'.$tipoMantenimiento.'</td>
