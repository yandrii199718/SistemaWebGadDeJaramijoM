...
$html = '<div></div><table cellpadding="4" style="border: 1px solid '.$colorE.'; font-size: 12px;">
		<tr>
			...
			<th width="8%" style="border: 1px solid '.$colorE.';">Usuario</th>
			<th width="8%" style="border: 1px solid '.$colorE.';">Dni</th>
			<th width="15%" style="border: 1px solid '.$colorE.';">Nombres</th>
			<th width="20%" style="border: 1px solid '.$colorE.';">Correo</th>
			<th width="8%" style="border: 1px solid '.$colorE.';">Telefono</th>
			<th width="12%" style="border: 1px solid '.$colorE.';">Cargo</th>
			<th width="12%" style="border: 1px solid '.$colorE.';">Area</th>
			<th width="12%" style="border: 1px solid '.$colorE.';">Sede</th>
		</tr>';
...
foreach($usuarios as $key => $usuario){
  ...
  $usuario = $usuarios[$key]["usuario"];
  $dni = $usuarios[$key]["dni"];
  $nombres = $usuarios[$key]['nombres'];
  $email = $usuarios[$key]['email'];
  $telefono = $usuarios[$key]['telefono'];
  $cargo = $usuarios[$key]['nombre_cargo'];
  $area = $usuarios[$key]['nombre_area'];
  $sede = $usuarios[$key]['nombre_sede'];


  $html .= '<tr style="font-size: 12px; color: #2E2E2E;">
    <td style="border: 1px solid '.$colorE.';">'.$key.'</td>
    <td style="border: 1px solid '.$colorE.';">'.$usuario.'</td>
    <td style="border: 1px solid '.$colorE.';">'.$dni.'</td>
    ...
    <td style="border: 1px solid '.$colorE.';">'.$cargo.'</td>
    <td style="border: 1px solid '.$colorE.';">'.$area.'</td>
    <td style="border: 1px solid '.$colorE.';">'.$sede.'</td>
  </tr>';
